<template>
    <li>
        <template v-if="item.child">
            <div class="relative flex items-center gap-2.5 rounded-2xl p-3"
                @click.prevent="handleItemClick" :class="{
                    'bg-slate-300': sidebarStore.click === item.label
                }">
            <span v-html="item.icon"></span>

            {{ item.label }}
            <svg v-if="item.child" class="absolute right-4 top-1/2 -translate-y-1/2 fill-current"
                :class="{ 'rotate-180': sidebarStore.click === item.label }" width="20" height="20" viewBox="0 0 20 20"
        ...
        <template v-else>
            <Link :href="item.route"class="relative flex items-center gap-2.5 p-3 rounded-2xl"
                @click.prevent="handleItemClick('singel')" :class="{
                    'bg-slate-600 text-white ': sidebarStore.page === item.label
                }">
            <span v-html="item.icon"></span>
        ...
        <div class="py-2" v-if="item.child" v-show="sidebarStore.click === item.label">
            <SidebarMenuDropdown  :items="item.child" :page="item.label" :currentPage="currentPage">
            </SidebarMenuDropdown>
        </div>
    </li>
</template>
...
const handleItemClick = (e) => {
    ...
    if(e ==='singel') {
        ...
        const opendropdown = sidebarStore.click === sidebarStore.page ? '' : props.item.label
